...
import com.atividadepoint.atvpontuada.service.FuncionarioService;
...
public class FuncionarioController {
    ...
    public ResponseEntity<?> criar(@Valid @RequestBody Funcionario funcionario) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(Map.of("mensagem", mensagem, "funcionario", novo));
    ...
    public ResponseEntity<?> atualizar(@PathVariable Long id, @Valid @RequestBody Funcionario funcionario) {
        try {
            ...
            String mensagem = "Funcionário " + atualizado.getNome() + " atualizado com sucesso";
            return ResponseEntity.ok(Map.of("mensagem", mensagem, "funcionario", atualizado));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("erro", e.getMessage()));
        }
